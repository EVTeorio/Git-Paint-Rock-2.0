...
vi_vars <- metrics  # Same list as metrics here
...
# Precompute canopy means for grouped inputs
...
for (i in 1:50) {
  ...
  # ========== One-vs-Rest Models for Grouped Feature Sets ==========
  ...
  for (group_name in names(model_inputs)) {
    ...
    classes_g <- unique(c(train_data$SpeciesID, test_data$SpeciesID))
    train_data$SpeciesID <- factor(train_data$SpeciesID, levels = classes_g)
    test_data$SpeciesID <- factor(test_data$SpeciesID, levels = classes_g)

    group_results <- list()

    for (cls in levels(train_data$SpeciesID)) {
      cat("Grouped:", group_name, " - class:", cls, "\n")
      ...
      cm_group <- confusionMatrix(preds_class, factor(test_bin$target, levels = c(0,1)))

      precision <- cm_group$byClass["Pos Pred Value"]
      recall <- cm_group$byClass["Sensitivity"]
      f1 <- 2 * (precision * recall) / (precision + recall)

      group_results[[cls]] <- list(
        confusion = cm_group,
        accuracy = cm_group$overall["Accuracy"],
        precision = precision,
        recall = recall,
        f1 = f1,
        importance = rf_group_model$variable.importance
      )
    }
    grouped_accuracy_results[[paste0("Sample_", i)]][[group_name]] <- group_results
  }
}
...
saveRDS(grouped_accuracy_results, "E:/Results/One_vs_Rest_Metric Importance/OneVsRest_Grouped_RF_accuracy_all_metrics.rds")
